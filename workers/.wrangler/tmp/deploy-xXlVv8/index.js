...
async function sendSmsCode(request, env) {
  try {
    ...
    try {
      ...
      console.log("[EmailJS] \u53D1\u9001\u8BF7\u6C42\u4F53:", JSON.stringify(emailjsBody));
      ...
      if (!emailjsResponse.ok) {
        ...
        throw new Error(`\u90AE\u4EF6\u53D1\u9001\u5931\u8D25: ${emailjsResponse.status} ${responseText}`);
      }
      console.log("[EmailJS] \u90AE\u4EF6\u53D1\u9001\u6210\u529F");
    } catch (emailjsError) {
      console.error("[EmailJS] \u53D1\u9001\u90AE\u4EF6\u51FA\u9519:", emailjsError);
      return addCorsHeaders(errorResponse("\u90AE\u4EF6\u53D1\u9001\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5", 500));
    }
    const id = generateId();
