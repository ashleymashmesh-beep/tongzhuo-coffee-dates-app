...
async function searchPlaces(request, env) {
  try {
    ...
    console.log("[AMAP] \u641C\u7D22\u5730\u70B9:", { keyword, city });
    const amapUrl = `https://restapi.amap.com/v3/place/text?key=${env.AMAP_KEY}&keywords=${encodeURIComponent(keyword)}&types=050100&city=${encodeURIComponent(city)}&pageSize=20&page=1&extensions=all`;
    console.log("[AMAP] \u8BF7\u6C42 URL:", amapUrl.replace(env.AMAP_KEY, "***"));
    const amapResponse = await fetch(amapUrl);
    console.log("[AMAP] \u54CD\u5E94\u72B6\u6001:", amapResponse.status, amapResponse.statusText);
    const responseText = await amapResponse.text();
    console.log("[AMAP] \u539F\u59CB\u54CD\u5E94\u5185\u5BB9:", responseText);
    let amapData;
    try {
    ...
    console.log("[AMAP] \u89E3\u6790\u540E\u6570\u636E:", JSON.stringify(amapData, null, 2));
    if (amapData.status !== "1") {
      console.error("[AMAP] API \u8FD4\u56DE\u9519\u8BEF:", amapData.info, amapData.infocode);
      return addCorsHeaders(jsonResponse({
        success: false,
        message: "\u641C\u7D22\u5931\u8D25: " + (amapData.info || "\u672A\u77E5\u9519\u8BEF"),
        error: {
          status: amapData.status,
          info: amapData.info,
          infocode: amapData.infocode,
          raw: responseText
        }
      }, 500));
    }
    if (!amapData.pois || amapData.pois.length === 0) {
      console.log("[AMAP] \u65E0\u641C\u7D22\u7ED3\u679C");
      return addCorsHeaders(jsonResponse({
        success: true,
        data: []
      }));
    }
    const places = amapData.pois.map((poi) => ({
      id: poi.id,
      name: poi.name,
      address: poi.address || "",
      city: poi.cityname || "",
    ...
    console.log("[AMAP] \u8FD4\u56DE\u5730\u70B9\u6570\u91CF:", places.length);
    return addCorsHeaders(jsonResponse({
      success: true,
      data: places
    }));
