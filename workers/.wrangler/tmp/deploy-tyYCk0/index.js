...
async function sendSmsCode(request, env2) {
  try {
    ...
    const recentCode = await env2.DB.prepare(
    ...
    if (recentCode) {
      return errorResponse("\u53D1\u9001\u592A\u9891\u7E41\uFF0C\u8BF7\u7A0D\u540E\u518D\u8BD5", 429);
    }
...
async function verifySmsCode(request, env2) {
  try {
...
async function getUser(request, env2, id) {
  try {
...
async function updateUser(request, env2, id) {
  try {
...
async function createEvent(request, env2) {
  try {
    const data = await request.json();
    const {
      creator_id,
    ...
    if (max_people !== 2 && max_people !== 3) {
      return errorResponse("\u4EBA\u6570\u4E0A\u9650\u53EA\u80FD\u662F 2 \u6216 3");
    }
    const id = generateId();
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await env2.DB.prepare(`
...
async function getEvents(request, env2) {
  try {
    ...
    const events = await env2.DB.prepare(query).bind(...params).all();
    let response = jsonResponse({ success: true, data: events.results || [] });
    return setCorsHeaders(request, response);
  } catch (error) {
...
async function getEvent(request, env2, id) {
  try {
    const event = await env2.DB.prepare(
      "SELECT * FROM events WHERE id = ?"
    ).bind(id).first();
    if (!event) {
      return errorResponse("\u6D3B\u52A8\u4E0D\u5B58\u5728", 404);
    }
    const signupCount = await env2.DB.prepare(
...
async function updateEvent(request, env2, id) {
  try {
...
async function createSignup(request, env2) {
  try {
...
async function getSignups(request, env2, eventId) {
  try {
    const signups = await env2.DB.prepare(`
...
__name(getSignups, "getSignups");
async function deleteSignup(request, env2, id) {
  try {
    await env2.DB.prepare(
...
async function createCheckin(request, env2) {
  try {
    const data = await request.json();
...
async function getCheckins(request, env2) {
  try {
    const url = new URL(request.url);
    ...
    if (!userId) {
      return errorResponse("\u7F3A\u5C11 user_id \u53C2\u6570");
    }
    ...
    const checkins = await env2.DB.prepare(query).bind(...params).all();
    let response = jsonResponse({ success: true, data: checkins.results || [] });
    return setCorsHeaders(request, response);
  } catch (error) {
...
async function getCheckin(request, env2, id) {
  try {
    const checkin = await env2.DB.prepare(
      "SELECT * FROM checkins WHERE id = ?"
    ).bind(id).first();
    if (!checkin) {
      return errorResponse("\u6253\u5361\u4E0D\u5B58\u5728", 404);
    }
...
async function deleteCheckin(request, env2, id) {
  try {
    await env2.DB.prepare(
...
async function createEncounter(request, env2) {
  try {
    const data = await request.json();
...
async function getEncounters(request, env2) {
  try {
    const url = new URL(request.url);
    ...
    const encounters = await env2.DB.prepare(`
      SELECT * FROM encounters
      WHERE user_id_1 = ? AND user_id_2 = ?
      ORDER BY date DESC
    `).bind(ids[0], ids[1]).all();
    let response = jsonResponse({ success: true, data: encounters.results || [] });
    return setCorsHeaders(request, response);
  } catch (error) {
...
async function createReview(request, env2) {
  try {
    const data = await request.json();
    const { from_user_id, to_user_id, event_id, rating, comment } = data;
    if (rating < 1 || rating > 5) {
      return errorResponse("\u8BC4\u5206\u5FC5\u987B\u57281-5\u4E4B\u95F4");
    }
    const id = generateId();
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await env2.DB.prepare(`
...
async function getReviews(request, env2) {
  try {
    ...
    if (!toUserId) {
      return errorResponse("\u7F3A\u5C11 to_user_id \u53C2\u6570");
    }
    const reviews = await env2.DB.prepare(`
...
var index_default = {
  async fetch(request, env2, ctx) {
    ...
    if (path === "/api/sms/send" && request.method === "POST") {
      return sendSmsCode(request, env2);
    }
    if (path === "/api/sms/verify" && request.method === "POST") {
      return verifySmsCode(request, env2);
    }
    if (path.startsWith("/api/users/") && request.method === "GET") {
      const id = path.split("/")[3];
      return getUser(request, env2, id);
    }
    if (path.startsWith("/api/users/") && request.method === "PUT") {
      const id = path.split("/")[3];
      return updateUser(request, env2, id);
    }
    if (path === "/api/events" && request.method === "GET") {
      return getEvents(request, env2);
    }
    if (path === "/api/events" && request.method === "POST") {
      return createEvent(request, env2);
    }
    if (path.startsWith("/api/events/") && request.method === "GET") {
      const id = path.split("/")[3];
      if (path.split("/")[4] === "signups") {
        return getSignups(request, env2, id);
      }
      return getEvent(request, env2, id);
    }
    if (path.startsWith("/api/events/") && request.method === "PUT") {
      const id = path.split("/")[3];
      return updateEvent(request, env2, id);
    }
    if (path === "/api/signups" && request.method === "POST") {
      return createSignup(request, env2);
    }
    if (path.startsWith("/api/signups/") && request.method === "DELETE") {
      const id = path.split("/")[3];
      return deleteSignup(request, env2, id);
    }
    if (path === "/api/checkins" && request.method === "GET") {
      return getCheckins(request, env2);
    }
    if (path === "/api/checkins" && request.method === "POST") {
      return createCheckin(request, env2);
    }
    if (path.startsWith("/api/checkins/") && request.method === "GET") {
      const id = path.split("/")[3];
      return getCheckin(request, env2, id);
    }
    if (path.startsWith("/api/checkins/") && request.method === "DELETE") {
      const id = path.split("/")[3];
      return deleteCheckin(request, env2, id);
    }
    if (path === "/api/encounters" && request.method === "GET") {
      return getEncounters(request, env2);
    }
    if (path === "/api/encounters" && request.method === "POST") {
      return createEncounter(request, env2);
    }
    if (path === "/api/reviews" && request.method === "GET") {
      return getReviews(request, env2);
    }
    if (path === "/api/reviews" && request.method === "POST") {
      return createReview(request, env2);
    }
