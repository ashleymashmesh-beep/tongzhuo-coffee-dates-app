...
async function sendSmsCode(request, env) {
  ...
  try {
    ...
    if (!email || typeof email !== "string") {
      return setCorsHeaders(errorResponse("\u90AE\u7BB1\u4E0D\u80FD\u4E3A\u7A7A"), origin);
    }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
      return setCorsHeaders(errorResponse("\u90AE\u7BB1\u683C\u5F0F\u4E0D\u6B63\u786E"), origin);
    }
    ...
    if (recentCode) {
      return setCorsHeaders(errorResponse("\u53D1\u9001\u592A\u9891\u7E41\uFF0C\u8BF7\u7A0D\u540E\u518D\u8BD5", 429), origin);
    }
    ...
    return setCorsHeaders(errorResponse("\u53D1\u9001\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5", 500), origin);
  }
}
...
async function verifySmsCode(request, env) {
  ...
  try {
    ...
    if (!email || !code) {
      return setCorsHeaders(errorResponse("\u90AE\u7BB1\u548C\u9A8C\u8BC1\u7801\u4E0D\u80FD\u4E3A\u7A7A"), origin);
    }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
      return setCorsHeaders(errorResponse("\u90AE\u7BB1\u683C\u5F0F\u4E0D\u6B63\u786E"), origin);
    ...
    if (!record) {
      return setCorsHeaders(errorResponse("\u9A8C\u8BC1\u7801\u9519\u8BEF\u6216\u5DF2\u8FC7\u671F"), origin);
    }
    await env.DB.prepare(
    ...
    return setCorsHeaders(errorResponse("\u9A8C\u8BC1\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5", 500), origin);
  }
}
...
async function getUser(request, env, id) {
  ...
  try {
    ...
    if (!user) {
      return setCorsHeaders(errorResponse("\u7528\u6237\u4E0D\u5B58\u5728", 404), origin);
    }
    return setCorsHeaders(jsonResponse({ success: true, data: user }), origin);
  } catch (error) {
    console.error("\u83B7\u53D6\u7528\u6237\u5931\u8D25:", error);
    return setCorsHeaders(errorResponse("\u83B7\u53D6\u5931\u8D25", 500), origin);
  }
}
...
async function updateUser(request, env, id) {
  ...
  try {
    ...
    if (updates.length === 0) {
      return setCorsHeaders(errorResponse("\u6CA1\u6709\u8981\u66F4\u65B0\u7684\u5B57\u6BB5"), origin);
    }
    values.push(id);
    await env.DB.prepare(
      `UPDATE users SET ${updates.join(", ")} WHERE id = ?`
    ).bind(...values).run();
    return setCorsHeaders(jsonResponse({ success: true, message: "\u66F4\u65B0\u6210\u529F" }), origin);
  } catch (error) {
    console.error("\u66F4\u65B0\u7528\u6237\u5931\u8D25:", error);
    return setCorsHeaders(errorResponse("\u66F4\u65B0\u5931\u8D25", 500), origin);
  }
}
...
async function createEvent(request, env) {
  ...
  try {
    ...
    if (max_people !== 2 && max_people !== 3) {
      return setCorsHeaders(errorResponse("\u4EBA\u6570\u4E0A\u9650\u53EA\u80FD\u662F 2 \u6216 3"), origin);
    }
    const id = generateId();
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await env.DB.prepare(`
    ...
    return setCorsHeaders(jsonResponse({ success: true, data: { id } }), origin);
  } catch (error) {
    console.error("\u521B\u5EFA\u6D3B\u52A8\u5931\u8D25:", error);
    return setCorsHeaders(errorResponse("\u521B\u5EFA\u5931\u8D25", 500), origin);
  }
}
...
async function getEvents(request, env) {
  ...
  try {
    ...
    return setCorsHeaders(jsonResponse({ success: true, data: result.results || [] }), origin);
  } catch (error) {
...
async function getEvent(request, env, id) {
  ...
  try {
    ...
    if (!event) {
      return setCorsHeaders(errorResponse("\u6D3B\u52A8\u4E0D\u5B58\u5728", 404), origin);
    }
...
async function updateEvent(request, env, id) {
  ...
  try {
    ...
    if (!["open", "full", "cancelled", "done"].includes(status)) {
      return setCorsHeaders(errorResponse("\u65E0\u6548\u7684\u72B6\u6001"), origin);
    }
    await env.DB.prepare(
...
async function getEventSignups(request, env, eventId) {
  ...
  try {
    ...
    `).bind(eventId).all();
    return setCorsHeaders(jsonResponse({ success: true, data: signups.results || [] }), origin);
  } catch (error) {
...
async function createSignup(request, env) {
  ...
  try {
    ...
    if (event.status !== "open") {
      return setCorsHeaders(errorResponse("\u6D3B\u52A8\u5DF2\u5173\u95ED"), origin);
    }
    const existing = await env.DB.prepare(
      "SELECT * FROM signups WHERE event_id = ? AND user_id = ?"
    ).bind(event_id, user_id).first();
    if (existing) {
      return setCorsHeaders(errorResponse("\u5DF2\u62A5\u540D"), origin);
    }
    const signupCount = await env.DB.prepare(
      "SELECT COUNT(*) as count FROM signups WHERE event_id = ?"
    ).bind(event_id).first();
    if ((signupCount?.count || 0) >= event.max_people) {
      return setCorsHeaders(errorResponse("\u6D3B\u52A8\u5DF2\u6EE1"), origin);
    }
    await env.DB.prepare(
      "INSERT INTO signups (id, event_id, user_id, created_at) VALUES (?, ?, ?, ?)"
    ...
    return setCorsHeaders(jsonResponse({ success: true, message: "\u62A5\u540D\u6210\u529F" }), origin);
  } catch (error) {
    console.error("\u62A5\u540D\u5931\u8D25:", error);
    return setCorsHeaders(errorResponse("\u62A5\u540D\u5931\u8D25", 500), origin);
  }
}
...
async function deleteSignup(request, env, id) {
  ...
  try {
    ...
    return setCorsHeaders(jsonResponse({ success: true, message: "\u53D6\u6D88\u6210\u529F" }), origin);
  } catch (error) {
    console.error("\u53D6\u6D88\u62A5\u540D\u5931\u8D25:", error);
    return setCorsHeaders(errorResponse("\u53D6\u6D88\u5931\u8D25", 500), origin);
  }
}
...
async function getCheckins(request, env) {
  ...
  try {
    ...
    if (!userId) {
      return setCorsHeaders(errorResponse("\u7F3A\u5C11 user_id \u53C2\u6570"), origin);
    }
...
async function getCheckin(request, env, id) {
  ...
  try {
    ...
    if (!checkin) {
      return setCorsHeaders(errorResponse("\u6253\u5361\u4E0D\u5B58\u5728", 404), origin);
    }
    return setCorsHeaders(jsonResponse({ success: true, data: checkin }), origin);
  } catch (error) {
...
async function deleteCheckin(request, env, id) {
  ...
  try {
    ...
    return setCorsHeaders(jsonResponse({ success: true, message: "\u5220\u9664\u6210\u529F" }), origin);
  } catch (error) {
    console.error("\u5220\u9664\u6253\u5361\u5931\u8D25:", error);
    return setCorsHeaders(errorResponse("\u5220\u9664\u5931\u8D25", 500), origin);
  }
}
...
async function createReview(request, env) {
  ...
  try {
    ...
    if (rating < 1 || rating > 5) {
      return setCorsHeaders(errorResponse("\u8BC4\u5206\u5FC5\u987B\u57281-5\u4E4B\u95F4"), origin);
    }
    const id = generateId();
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await env.DB.prepare(`
...
async function getReviews(request, env) {
  ...
  try {
    ...
    if (!toUserId) {
      return setCorsHeaders(errorResponse("\u7F3A\u5C11 to_user_id \u53C2\u6570"), origin);
    }
    ...
    `).bind(toUserId).all();
    return setCorsHeaders(jsonResponse({ success: true, data: reviews.results || [] }), origin);
  } catch (error) {
...
var index_default = {
  async fetch(request, env, ctx) {
    ...
    if (eventMatch && request.method === "GET") {
      return getEvent(request, env, eventMatch[1]);
    }
    if (eventMatch && request.method === "PUT") {
      return updateEvent(request, env, eventMatch[1]);
    }
    ...
    if (checkinMatch && request.method === "GET") {
      return getCheckin(request, env, checkinMatch[1]);
    }
    if (checkinMatch && request.method === "DELETE") {
      return deleteCheckin(request, env, checkinMatch[1]);
    }
    ...
    return setCorsHeaders(errorResponse("Not found", 404), origin);
  }
};
export {
