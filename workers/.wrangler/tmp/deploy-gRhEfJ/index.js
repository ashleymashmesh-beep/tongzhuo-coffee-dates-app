...
async function searchPlaces(request, env) {
  try {
    ...
    console.log("[DEBUG] \u641C\u7D22\u5730\u70B9:", { keyword, city });
    const amapUrl = `https://restapi.amap.com/v3/place/text?key=${env.AMAP_KEY}&keywords=${encodeURIComponent(keyword)}&types=050100&city=${encodeURIComponent(city)}&pageSize=20&page=1`;
    const amapResponse = await fetch(amapUrl);
    const amapData = await amapResponse.json();
    console.log("[DEBUG] \u9AD8\u5FB7 API \u54CD\u5E94:", amapData);
