...
async function sendSmsCode(request, env2) {
  try {
    ...
    if (!email || typeof email !== "string") {
      ...
      return setCorsHeaders(request, response2);
    }
...
async function getEventSignups(request, env2, eventId) {
  try {
...
var index_default = {
  async fetch(request, env2, ctx) {
    ...
    if (path.startsWith("/api/events/") && request.length === 4 && request.method === "GET") {
      const id = path.split("/")[3];
    ...
    if (path.startsWith("/api/events/") && path.split("/").length === 5 && request.method === "GET") {
      const id = path.split("/")[3];
      return getEventSignups(request, env2, id);
    }
    ...
    if (path.startsWith("/api/checkins/") && request.length === 4 && request.method === "GET") {
      const id = path.split("/")[3];
      return getCheckin(request, env2, id);
    }
    if (path.startsWith("/api/checkins/") && path.length === 4 && request.method === "DELETE") {
      const id = path.split("/")[3];
